...
**Status:** Phase 0 ✅ (2026-05-20). Phase 1 ✅ — `user.Language` field + PATCH `/v1/auth/{tier}/me`. Phase 2 setup ✅ — `internal/shared/errcode` (renamed from `errs`) + `AuthEmailInUse` worked example. Phase 3 ✅ — `react-i18next` bootstrapped in `frontend-admin/`, EN default + EN fallback, detector chain `cookie → navigator`, `orkestra_admin_lang` cookie, typed `t()` keys from day 1, `useLanguageSync()` hook reconciles `user.language` → `i18n.changeLanguage` on auth state change, locales seeded with `app.name` only. Phase 4 (per-module string extraction) is the long-tail next step. IT reviewer assignment still open.
**Owner:** Salvatore
