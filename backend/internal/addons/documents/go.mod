// Orkestra Documents addon — Italian/EU-style PDF generation via the
// Gotenberg sidecar. Hosted in-tree at backend/internal/addons/documents/
// as its own Go module so the same source can be consumed by the
// orkestra monolith AND extracted to a standalone repository
// (orkestra-cc/orkestra-addon-documents) at the same import path. The
...
module github.com/orkestra-cc/orkestra-addon-documents
