...
export const subscriptionsApi = baseApi.injectEndpoints({
  endpoints: builder => ({
    ...
    >({
      query: params =>
        `/v1/subscriptions/services${params ? buildQS(params) : ''}`,
      providesTags: result =>
        result?.items
          ? [
              ...result.items.map(({ uuid }) => ({
                type: 'SubscriptionService' as const,
                id: uuid
              })),
    ...
    getSubscriptionService: builder.query<
    ...
    createSubscriptionService: builder.mutation<
      { body: SubscriptionService },
      CreateServiceInput
    >({
      query: body => ({
        url: '/v1/subscriptions/services',
        method: 'POST',
        body
      }),
    ...
    >({
      query: ({ id, patch }) => ({
        url: `/v1/subscriptions/services/${id}`,
        method: 'PATCH',
        body: patch
      }),
    ...
    >({
      query: params =>
        `/v1/subscriptions/subscriptions${params ? buildQS(params) : ''}`,
      providesTags: result =>
        result?.items
          ? [
              ...result.items.map(({ uuid }) => ({
    ...
    >({
      query: body => ({
        url: '/v1/subscriptions/subscriptions',
        method: 'POST',
        body
      }),
    ...
    reactivateSubscription: builder.mutation<{ body: Subscription }, string>({
      query: id => ({
        url: `/v1/subscriptions/subscriptions/${id}/reactivate`,
        method: 'POST'
      }),
      invalidatesTags: (_r, _e, id) => [
        { type: 'Subscription', id },
    ...
    retryCharge: builder.mutation<{ body: Subscription }, string>({
      query: id => ({
        url: `/v1/subscriptions/subscriptions/${id}/retry-charge`,
        method: 'POST'
      }),
      invalidatesTags: (_r, _e, id) => [
        { type: 'Subscription', id },
    ...
    listSubscriptionInvoices: builder.query<
      ListResponse<SubscriptionInvoice>,
      string
    >({
    ...
    >({
      query: ({ id, limit }) =>
        `/v1/subscriptions/subscriptions/${id}/activity${buildQS({ limit })}`,
